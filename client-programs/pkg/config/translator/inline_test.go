...
// same externalTLSTermination assertion as the cloud kinds: no wildcard
// certificate is referenced and the inline ingress protocol is https.
func TestTranslateInline_ExternalTLSTermination_SetsInlineIngressProtocol(t *testing.T) {
...
`))
	...
	if got, want := ingress["protocol"], "https"; got != want {
		t.Errorf("inline.ingress.protocol = %v, want %v", got, want)
	}
	if _, present := ingress["wildcardCertificateSecretRef"]; present {
		t.Errorf("wildcardCertificateSecretRef unexpectedly present under external TLS termination: %v", ingress)
	}
