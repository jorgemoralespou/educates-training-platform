...
type PlatformDeleteV4Options struct {
	Kubeconfig string
	Context    string
...
func (p *ProjectInfo) NewAdminPlatformDeleteV4Cmd() *cobra.Command {
	var o PlatformDeleteV4Options

	c := &cobra.Command{
		Args:   cobra.NoArgs,
		Use:    "delete-v4",
		Short:  "v4 walking-skeleton delete: drain CRs + helm uninstall (experimental)",
		Hidden: true,
		Long: `Walking-skeleton implementation of the v4 uninstall path.
The reverse of 'admin platform deploy-v4':
  ...
  4. delete EducatesClusterConfig → wait gone
       (the ECC finalizer drains kyverno, external-dns, contour,
        cert-manager and the CustomCA Secret copy in reverse order)
...
Unlike deploy-v4, this command takes no --config / --local-config — the
...
educates-installer release. The kubeconfig flags suffice.`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			return p.runDeleteV4(cmd.Context(), cmd.OutOrStdout(), &o)
		},
	}
...
func (p *ProjectInfo) runDeleteV4(ctx context.Context, w io.Writer, o *PlatformDeleteV4Options) error {
