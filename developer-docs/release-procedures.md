...
* The `//go:embed` filenames and `<X>ChartVersion` constants in `installer/operator/vendored-charts/embed.go`, which name the five runtime subchart tarballs at the stamped version. The tarballs themselves are repackaged at that version too, but they are gitignored build output, so they are not part of the commit and have no `SHA256SUMS` entries. The upstream cluster-service charts (cert-manager, Contour, external-dns, Kyverno) are NOT touched — those are only ever re-vendored deliberately via `make vendor-charts`.
